...
class Challenges extends DataAccess{
	...
	*/
	public function createChallenge($userId,$partnerId,$gameId){
		...
		$vars = array(NULL,$gameId,$userId,$partnerId,$this->WAITING);
	...
	*/
	public function getChallengeStatus($challengeId){
		...
		//query to insert user details into the users table
		$query = "SELECT `status` FROM `challenges` WHERE `challenge_id` = ?";
	...
	*/
	public function getChallengeDetails($userId){
		...
		$query = "SELECT C.`challenge_id`,G. `name`, U.`name` as player1,U2. `name` as player2, C.`status` FROM `challenges` C , `users` U, `users` U2 , games G WHERE  (C.`player1_id` = ? OR C.`player2_id` = ? ) AND U.id_users = C.`player1_id` AND U2.id_users = C.`player2_id` AND C.`status` = ? AND G.`game_id` =  C.`game_id`";
	
		//build the vaariables array which holds the data to bind to the prepare statement.
		$vars = array($userId, $userId, $this->WAITING);
